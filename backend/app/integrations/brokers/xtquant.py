...
class XtQuantBrokerAdapter(BrokerAccountAdapter):
    def __init__(self, client_factory=None):
        self.client_factory = client_factory or self._default_client_factory

    def _default_client_factory(self, _config):
        raise RuntimeError("XtQuant local connector client is not configured")

    def _connect(self, config):
        _require_fields(config["config_public"], ["endpoint", "account_id"])
        client = self.client_factory(config)
        client.connect(
            config["config_public"]["endpoint"],
            config["config_public"]["account_id"],
            client_path=config["config_public"].get("client_path"),
        )
        return client
