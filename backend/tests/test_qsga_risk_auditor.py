...
def backtest_result(summary=None, trades=None):
    return {
        "summary": summary
        or {"maxDrawdown": -8.0, "maxPositionPct": 25, "turnoverOrdersPerDay": 1},
...
def test_risk_auditor_passes_historical_constraints():
    audit = audit_risk_constraints(
        load_fixture("trend_following_basic.json"), backtest_result()
    )
...
def test_risk_auditor_reports_repairable_drawdown_violation():
    audit = audit_risk_constraints(
        ...
        backtest_result(
            {"maxDrawdown": -20.0, "maxPositionPct": 25, "turnoverOrdersPerDay": 1}
        ),
    )

    assert audit["status"] == "violation"
    drawdown = next(
        check for check in audit["checks"] if check["constraint"] == "max_drawdown"
    )
...
def test_risk_auditor_checks_turnover_from_trades_when_summary_missing():
    ...
    turnover = next(
        ...
        if check["constraint"] == "turnover_orders_per_day"
    )
    assert turnover["status"] == "violation"
    assert turnover["observed"] == 3.0


def test_risk_auditor_fails_when_required_observations_are_missing():
    audit = audit_risk_constraints(
        load_fixture("trend_following_basic.json"),
        backtest_result({"maxDrawdown": -8.0, "maxPositionPct": 25}, trades=[]),
    )
    ...
    assert turnover["status"] == "not_run"
    result = verify_risk_audit(audit)
    assert not result.passed
    assert result.errors[0].code == "RISK_AUDIT_INCOMPLETE"
